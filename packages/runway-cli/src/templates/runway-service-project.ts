...
export class RunwayServiceProject extends typescript.TypeScriptProject {
  ...
  /**
   * SS-02, enforced where the config lives.
   *
   * Production deploys a digest, never a tag — a tag can be repointed, and
   * then production runs something no environment tested. The program cannot
   * refuse a tag on production because SS-01 forbids it from knowing which
   * stack it is, so the check runs in the generated repo's own suite, against
   * the file that drifts.
   *
   * projen-managed rather than a sample: deleting it brings it back on the
   * next `npx projen`, so opting out is a visible edit to `.projenrc.ts`
   * rather than a quiet `rm` — the same "work to opt out" the components use.
   */
  private addProductionImageGuard(): void {
    new TextFile(this, "test/production-image.test.ts", {
      lines: [
        '// ~~ Generated by projen. To modify, edit .projenrc.ts and run "npx projen".',
        "//",
        "// SS-02: production deploys a digest, never a tag. A tag can be repointed,",
        "// and then production runs something no environment tested. Two states are",
        "// legitimate: no image reference at all (nothing has been promoted yet),",
        "// and the imageDigest CI writes at promotion. This test failing is the",
        "// control working.",
        'import { readFileSync } from "node:fs";',
        'import { describe, expect, it } from "vitest";',
        "",
        'const config = readFileSync("infra/Pulumi.production.yaml", "utf-8")',
        '  .split("\\n")',
        '  .filter((line) => !line.trim().startsWith("#"))',
        '  .join("\\n");',
        "",
        'describe("SS-02: the production stack is digest-pinned", () => {',
        '  it("references no image tag", () => {',
        "    expect(config).not.toMatch(/imageTag:/);",
        "  });",
        "",
        '  it("pins any promoted image to a sha256 digest", () => {',
        '    const digest = /imageDigest:\\s*"?([^"\\s]+)"?/.exec(config);',
        "    if (digest !== null) {",
        "      expect(digest[1]).toMatch(/^sha256:[0-9a-f]{64}$/);",
        "    }",
        "  });",
        "});",
        "",
