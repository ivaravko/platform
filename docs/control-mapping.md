...
| EP-02 | The production deploy roles are granted only to the CI federated identity, scoped to one repository and its named refs (main for image pushes, the release tag prefix) | [Workload Identity Federation](https://docs.cloud.google.com/iam/docs/workload-identity-federation) | `ServiceEnvironment` | `EP-02: …` | — |
